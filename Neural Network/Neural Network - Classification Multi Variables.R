...
temp_test <- subset(mydata, select = c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width"))

# This will run the temp_test dataset through the NN we created on the
# training data.

speciesnet.results <- compute(speciesnet, temp_test)

# Let's have a look at what the neural network produced:

results <- data.frame(actual = testset[,6:8], prediction = speciesnet.results$net.result)
results[100:115, ]
